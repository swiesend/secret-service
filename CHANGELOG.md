...
- `Fixed`:
  - **Per-item unlock.** A provider may lock items individually — the spec says a client "should act as if it must unlock each item individually", and KeePassXC does exactly that with its per-item access confirmation. The library only ever unlocked the *collection*, so reading a locked item failed. `getSecret`/`withSecret` now unlock the item first when the provider reports it locked, and return empty if the unlock is refused rather than falling through to a read that cannot succeed. Reported and first fixed by @SvenMarquardt5772 [#45](https://github.com/swiesend/secret-service/issues/45), [#46](https://github.com/swiesend/secret-service/pull/46).
    - The unlock is attempted **only when the collection is already unlocked**, which is the genuine per-item condition. gnome-keyring reports every item as locked while its collection is locked, so without that guard a failed collection unlock — a dismissed prompt, a wrong stored password, `disablePrompt()` — would raise a *second* prompt for the keyring the user just refused. Verified against a live daemon and covered by `aLockedCollectionDoesNotTriggerASecondUnlockOfTheItem`.
    - Costs one extra `Locked` property read per `getSecret`/`withSecret`: the item's own. A second read, of the collection's, happens only when the item reports locked — so on a provider that does not lock items individually the check ends after the first. `getSecrets`/`withSecrets` are unaffected: they iterate the whole collection and never attempt a per-item unlock, because one prompt per item is not a reasonable thing to do to a user.
    - `unlockItem` now checks the prompt result. It previously ignored dismissal and could throw a `NullPointerException` out of a `boolean` method when the prompt timed out.
...
- `Added` — architecture:
  - **Maven reactor split into three artifacts** at one version: `de.swiesend:secret-service` (the classical client; only the build moved into a `core/` submodule — GAV unchanged), `de.swiesend:secret-service-hardened` (opt-in AES-256-GCM envelopes with HKDF-derived per-item DEKs, a pluggable `KeyMaterialProvider` SPI, optional Argon2id pepper-stretching, optional hybrid X25519 + ML-KEM-768 with persisted-and-destroyable epoch keypairs for forward secrecy), and `de.swiesend:secret-service-hardened-tpm2` (optional `Tpm2KeyMaterialProvider` + `Tpm2Provisioner` CLI for TPM-sealed peppers, classpath-only). All three target JDK 25 (see the baseline note above). Parent pom is a BOM pinning all three at one version.
...
- `Security` / `Changed` — hardening & simplification round 2:
  - **Envelope format v3** (breaking, alpha only): the AEAD and KDF are now agile like the KEM already was — authenticated `aead_id` and `kdf_id` selector bytes let a new cipher/KDF land without a format bump. **ChaCha20-Poly1305** ships as a second AEAD alongside AES-256-GCM, selectable via `HardenedCollection.Builder.aead(AeadId)`. `fromBytes` accepts only v3 (v1/v2 rejected). The dead `KEM_ID_NONE` write path was removed. v3 is now the **frozen** wire format for the release line, pinned by a committed regression fixture; a future change requires a new `version` byte, not a v3 mutation.
  ...
  - **Testing**: the TPM tests now run in CI against a software TPM — a single `tpm-tests.yml` builds the Microsoft **ms-tpm-20-ref** simulator once and runs both the fast unit/integration TPM tests and the live-daemon `HardenedTpm2SystemTest` (with a silent-skip guard); coverage-guided **Jazzer** fuzzing of the envelope parser; RFC 5869 / AEAD known-answer tests; a committed **v3 wire-format fixture**; migration-failure, anti-rollback-anchor error-vs-rollback, and a **thread-safety/concurrency** test; a manual throughput benchmark.
  - **Performance**: shared `SecureRandom` (was 3–5 fresh instances per write) and a dropped redundant buffer in the UTF-8 path.
- `Fixed` — key-destruction and lifecycle defects found in review:
  - **`rotateEpoch` could destroy every epoch key without proof.** A failed item enumeration was indistinguishable from an empty one, so rotation could report success having rewrapped nothing, and — with a `CollectionInterface` that returns an empty list — could call `retainOnly` with an epoch it did not hold, emptying the keystore and making every item permanently undecryptable. Rotation now commits the new epoch durably *before* rewrapping, re-enumerates to verify every surviving item names it, and refuses to destroy anything otherwise; `retainOnly` refuses an epoch it does not hold. Root cause fixed in `core`: `Collection.getItems()` no longer maps a successful-but-empty search to `Optional.empty()` (the legacy `SimpleCollection.getItems` keeps returning `null` for empty).
  - **Equal-generation keystores deleted each other.** Two writers over one collection could reach the same generation, and the loser — chosen by daemon-returned item order — was hard-deleted along with its epoch keys. Only a strictly lower generation is now treated as superseded. A collection is documented as **single-writer**.
  - **An anti-rollback refusal is now sticky.** Refusing a below-floor keystore used to leave the instance writable, so the next write forked a second keystore and the following load destroyed the genuine snapshot — turning the refusal into the destruction it was meant to prevent. Writes after a refusal now throw.
  - **Epoch identity moved from the instance to the collection** (keystore blob **v3**, breaking for alpha keystores; v1/v2 blobs still load). Each `HardenedCollection` used to mint a fresh UUID epoch and add a keypair on first write — unbounded growth in a single D-Bus item. Instances now discover and reuse the collection's recorded epoch; `status().epochId()` reports `(unresolved)` until the first write. **Note:** because the current epoch travels in the keystore, a keystore rollback now also redirects future writes onto the resurrected epoch — mitigated by a `GenerationAnchor`.
  - **`close()` no longer closes objects you supplied** (breaking). It used to close the caller's `KeyMaterialProvider`, `GenerationAnchor` and the wrapped `CollectionInterface` — the last tearing down a possibly-shared D-Bus session. Opt back in per object with `Builder.ownsProvider/ownsAnchor/ownsWrapped`. `getPepper()` also moved inside the guarded blocks, so a closed provider yields `Optional.empty()` instead of throwing through an `Optional`-returning API.
  - **Enabling `enablePostQuantum` over an existing collection wrote unreadable items.** `kem_id` was stamped from the runtime PQ preference while the actual encapsulation fell back to classical (the recorded epoch, minted before PQ was on, has no ML-KEM half), so the envelope advertised a PQ hybrid over a classical-only `kem_ct` and never decrypted again — silently, since `createItem` still returned a path. `kem_id` now follows what was actually encapsulated, and an existing epoch gains its ML-KEM half when PQ becomes available.
  - **One `GenerationAnchor` may back only one collection**, and a second is now refused at construction. The floor is global while each collection keeps its own generation, so sharing an anchor pushed each collection below the other's floor and bricked both (fail-closed on reads *and* writes). `GenerationAnchor.scopeKey()` identifies the counter; `Tpm2GenerationAnchor` returns its NV index, so two distinct objects on one counter are caught too.
  - `rotateEpoch`'s pre-destruction verification now reads the epoch from the **AEAD-authenticated envelope** rather than the plaintext, daemon-mutable `hardened.epoch` attribute.
  - **`acknowledgeSecurityTheater(true)` renamed to `acknowledgeSameUidExposure(true)`** (breaking), with `SecurityTheaterException` → `SameUidExposureException` and `ThreatCoverage.isSecurityTheaterVsSameUid()` → `hasNoSameUidProtection()`. The old name delivered a verdict rather than information: it named neither the attacker class that is uncovered (same-UID) nor the protection that remains real (offline, cross-UID and daemon-level access still see only ciphertext) — and it carried no Javadoc at all. The setter now documents what is given up, what still holds, when the flag is legitimate and what to use instead; the exception message says the same. New `Builder.suppressSameUidExposureWarning(boolean)` silences the per-instance WARN for CI/test log hygiene without touching the gate. The migration gates, which reused the same exception for an unrelated reason, now throw their own `MigrationNotPermittedException` (also an `IllegalStateException`, so existing catches still work).
  - **A failed ML-KEM import no longer destroys the key.** `deserialize` swallowed the exception and left the keypair null, and `serialize` then wrote a zero-length private — so running once on a JVM without an ML-KEM provider permanently destroyed every PQ epoch key on the next persist, even after returning to a PQ-capable JVM. The raw encoded bytes are now retained and written back verbatim, so the outage is fully recoverable; `upgradePqIfNeeded` gates on the stored bytes rather than on `mlkem == null`, so it can no longer mint over — and discard — a surviving public key.
  - **The anchor registry no longer leaks on a refused build.** It registered before the same-UID gate could throw, so a rejected `build()` left an entry that blamed a collection which was never constructed; and it held the wrapped collection strongly in static state, pinning a D-Bus session for the JVM lifetime. Registration moved to the end of the constructor and the reference is now weak.
  - **Rotation verifies over the full item set, classified by envelope magic.** `rewrapCovered` re-enumerated through the same daemon-mutable `hardened.version` filter it was meant not to trust, so an item hidden from that filter — by a hostile daemon or simply by a provider with unreliable `SearchItems` — had its epoch key destroyed unverified. It now enumerates everything and uses `Envelope.looksLikeEnvelope`. The same check stops migration double-enveloping an `SSv1` item whose attribute was stripped, and `rotateEpoch` now skips an item whose attributes it cannot read instead of rewrapping it.
  - **An unexecutable `kem_id` is refused rather than reported as tampering.** `KemId` gained `implemented()` (false for the reserved `x25519+hqc-192`), and the reader checks it alongside `aead_id`/`kdf_id` — the one selector byte that was missing the guard.
  - `CollectionInterface.getItems` documents the 3.0.0 contract change: `Optional.of(emptyList)` for a successful search with no matches, `Optional.empty()` only for a failed search.
  - **Documented Java snippets are now compiled in CI.** `.github/scripts/check_doc_snippets.py` extracts every ```java fence under `docs/`, compiles it against the built artifacts, and fails the docs workflow on any error — which also runs when `*/src/main/**` changes, since prose breaks when the API moves, not when the prose does. It immediately found that **all three usage quickstarts did not compile**: `SecretService.create()` returns `ServiceInterface` (not `SecretService`), `collection(...)` is on `SessionInterface` via `openSession()`, and `unlock()` is private. `docs/index.md` had it right, so the landing page and the usage pages disagreed.
  - Docs: `usage/core.md` grew from 3 documented methods to the full item lifecycle — find (`getItems` vs `search` and the five `SearchMode`s), update, delete, locking and prompts — plus an at-a-glance table for the rest, and the traps that are invisible in a signature (`getSecrets()` prompts and silently skips unreadable items; `isLocked()` returns `true` when there is no connection; `search` collapses a failed enumeration into an empty list).
  - Docs: a new **"Why these primitives"** section explains the crypto choices that were previously only described — why hybrid rather than PQ-only, why ML-KEM-768, why JDK-native ciphers only, when to prefer ChaCha20-Poly1305 over AES-256-GCM, and why HKDF rather than a password KDF for the per-item key.
  - Docs: resolved contradictions between the deployment decision aids (the desktop tree ranked `.deb` over Snap while the Snap section called Snap the only format pairing cleanly with the wrapper; two sibling branches fired at once; Flatpak was recommended for a path that drops the library). **Every A/B/C/D coverage verdict now has exactly one canonical home** (mechanism → the defense inventory; LUKS → its own page; distribution format → the format-vs-class table; backend → the class-by-class comparison), with every other page linking to it — the rule is stated in the security overview so it does not regress. That removed six duplicated per-format coverage tables from the desktop and CI pages, and the format table gained the class D column it had been missing. Removed a lossy restatement of the backend class table, corrected the class-A precondition for self-hosted runners, fixed two cross-links that pointed at pages not containing the claim, de-duplicated the LUKS coverage table, and removed the flattened self-referential links from both decision trees. Corrected the stale `hkdf` runtime dependency in `getting-started.md`.
  - Docs: `Tpm2Provisioner` now states that the sealed pepper is **base64-encoded**, not verbatim (an escrow footgun); the health check no longer suggests `Argon2KeyMaterialProvider` as a remedy for `sameUid=NONE`, which it cannot fix; `KeyMaterialProvider`, `Builder.generationAnchor` and `Tpm2GenerationAnchor` no longer promise the pre-flip `close()` propagation; the usage examples no longer demonstrate `withSecret(path, String::new)`, which defeats the zeroing guarantee they exist to teach.
