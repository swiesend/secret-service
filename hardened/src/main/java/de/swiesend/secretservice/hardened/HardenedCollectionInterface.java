...
public interface HardenedCollectionInterface extends AutoCloseable {
    ...
    /**
     * Releases resources this instance owns.
     *
     * <p><b>By default it owns nothing you passed in.</b> The wrapped {@code CollectionInterface},
     * the {@link KeyMaterialProvider} and any {@code GenerationAnchor} were constructed by the
     * caller and are left open, so a shared D-Bus session or a provider shared between collections
     * survives this call. Opt in per object via {@code Builder.ownsWrapped/ownsProvider/ownsAnchor}
     * if you would rather this instance close them for you.</p>
     *
     * <p>Closing is idempotent and never throws: failures from owned components are logged and
     * swallowed so one broken component cannot strand the others.</p>
     */
    @Override
    void close();
}
