...
public final class HybridKem {
     ...
     */
    public record Encapsulation(byte[] sharedSecret, byte[] kemCiphertext, boolean postQuantum) {
        /**
         * Whether an ML-KEM ciphertext is actually present in {@link #kemCiphertext()}. The caller
         * MUST stamp the envelope's {@code kem_id} from this, not from the runtime PQ preference:
         * an epoch minted before PQ was enabled carries no ML-KEM public key, so encapsulation
         * silently falls back to classical even when PQ is switched on.
         */
    ...
    public Encapsulation encapsulate(PublicKey epochX25519Public, PublicKey epochPqPublicOrNull) {
        ...
        return new Encapsulation(combined, kemCt, pqCt.length > 0);
    }
    ...
    public byte[] decapsulate(PrivateKey epochX25519Private,
        ...
        byte[] ssPq = new byte[0];
        // The READ path is driven by the envelope, never by postQuantumAvailable -- that field is
        // the write-side preference (preferPostQuantum && provider present). Gating on it meant
        // that turning enablePostQuantum back off silently dropped the ML-KEM half of an envelope
        // that carries one, producing a wrong combined secret and surfacing as "AEAD
        // authentication failed" -- i.e. reporting a config change as tampering. When the envelope
        // says hybrid we either decapsulate both halves or fail loudly with a distinguishable
        // error; we never silently derive a different secret.
        if (envelopeIsHybrid && pqCt.length > 0) {
            if (epochPqPrivateOrNull == null) {
                Arrays.fill(ssClassical, (byte) 0);
                throw new IllegalStateException(
                        "envelope is PQ-hybrid but its epoch has no ML-KEM private key; cannot decrypt");
            }
            if (!PqProviderBootstrap.ensurePqProvider()) {
                Arrays.fill(ssClassical, (byte) 0);
                throw new IllegalStateException(
                        "envelope is PQ-hybrid but no ML-KEM provider is available; cannot decrypt");
            }
            try {
                KEM kem = KEM.getInstance(PqProviderBootstrap.mlKem768Algorithm());
