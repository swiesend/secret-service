...
| **TOTP seed** (only when `STORED_STEP` or `LIVE_CODE` mode is on) | Per the configured `KeyMaterialProvider` | All TOTP-bound items become unreadable; non-TOTP items unaffected | Same as the pepper |
