...
The hardened surface has no `getSecret(String)` returning a raw `char[]`: an array handed to the
caller cannot be zeroed by the library, so plaintext access is callback-only.
