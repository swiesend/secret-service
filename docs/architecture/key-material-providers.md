...
The builder **refuses** a provider whose `ThreatCoverage` rates same-UID as `NONE` (e.g. `EnvVarKeyMaterialProvider`) unless the caller explicitly calls `acknowledgeSecurityTheater(true)` — so a weak dev provider can never be shipped to production silently.
...
| `close()` propagates to the provider (scrubs cached material) | `closePropagatesToProvider` |
