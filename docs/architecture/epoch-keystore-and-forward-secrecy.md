...
## The epoch belongs to the collection, not to the instance

The current epoch id is recorded **inside** the keystore blob (within the AES-GCM ciphertext, not in a D-Bus attribute — attributes are plaintext and daemon-mutable). Every `HardenedCollection` over a collection discovers and reuses it; only `rotateEpoch()` mints a new one.

This matters operationally: epochs would otherwise accumulate one per process start, in a single D-Bus item, shrinkable only by a fully-successful rotation. Resolution is lazy — the constructor performs no D-Bus I/O, so `status().epochId()` reports `(unresolved)` until the first write.

The trade-off is explicit: because the current epoch travels in the keystore, a keystore **rollback** now also redirects *future* writes onto the resurrected epoch, not merely past ones. The field is inside authenticated ciphertext so it cannot be tampered independently, and a configured [`GenerationAnchor`](anti-rollback-anchor.md) refuses a below-floor snapshot outright — which is the mitigation.

## Single writer per collection

A collection must have **one writer**. There is no writer identity in the blob, so two processes persisting concurrently can reach the same generation; the keystore now leaves an equal-generation peer snapshot alone (only a *strictly lower* generation is provably superseded and deleted) and logs loudly, rather than deleting a peer's epoch keys.
