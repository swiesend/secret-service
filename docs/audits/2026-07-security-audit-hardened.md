...
| F-3 | Largely fixed | Envelope **v2**: full-header AEAD associated data; item-id and TOTP mode/step embedded as authenticated fields and read from the envelope, not D-Bus attributes. The read path additionally compares the `hardened.item.id` attribute against the authenticated envelope id and fails closed on mismatch. **Accepted residual:** whole-envelope *relocation* by an attacker who copies both the body and the attribute onto another item remains possible — nothing stable binds an envelope to a storage slot in daemon-mutable storage (see the F-3 section). |
...
**Resolution and residual (v3).** The header-binding half is done: the AEAD associated data covers
the full envelope header, and the item id used on read comes from the authenticated envelope, so
*steering* the read via attributes is closed. The read path also compares the `hardened.item.id`
attribute against the authenticated envelope id and refuses on mismatch — which catches accidental
relocation, partial tampering and provider bugs. The comparison is deliberately **not** claimed as
a defense against the attacker this finding describes: a keyring-writer who relocates the envelope
body can copy the attribute in the same write, and both halves are then self-consistent. Envelopes
have no stable storage-slot identity to bind to (paths are provider-assigned and change on every
rewrap), so whole-envelope relocation inside daemon-mutable storage is an **accepted residual
risk**: the storage backend is outside this layer's integrity boundary, and a daemon-level writer
could equally delete or swap entire items.
...
- **G-6 — Honest self-assessment infrastructure.** `ThreatCoverage`, the `SameUidExposureException`
