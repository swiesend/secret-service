...
**Limitations.** Once the system boots and LUKS is unlocked, every other class (A/B/D) sees the decrypted contents. LUKS is a class-C mitigation only. In particular, "LUKS is unlocked by my TPM at boot" is NOT the same as "my application secret is sealed in my TPM" — both are useful, neither replaces the other. See [LUKS / full-disk encryption](full-disk-encryption.md).
