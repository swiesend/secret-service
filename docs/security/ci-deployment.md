...
See the [canonical coverage row](sample-configurations.md#distribution-format-vs-threat-class-summary) for the self-hosted runner. On a shared runner nothing
defends class A or B: sibling jobs read your `/proc/<pid>/environ`, so the mitigation is per-job
UID isolation, not anything the JAR can do.
...
Use this library's `Tpm2Availability.isAvailable()` preflight (from `secret-service-hardened-tpm2`; see [graceful fallback when TSS.Java is missing](../usage/tpm2.md#graceful-fallback-when-tssjava-is-missing)) to detect "no TPM here, fall back to file-based provider" gracefully.
...
See the [canonical coverage row](sample-configurations.md#distribution-format-vs-threat-class-summary) for the OCI container. The class-C `REAL` is earned
only by BuildKit secret mounts — a secret baked into any image layer stays there.
...
├── Yes -- typical GitHub Actions / GitLab.com / CircleCI
│   ├── OCI container + the action wrapper
│   ├── EnvVarKeyMaterialProvider reading a platform-injected secret
│   └── DO NOT reach for Secret Service or TPM: neither is present.
└── No -- a long-lived runner or controller
    ├── Self-hosted runner on a dedicated VM -> .deb/.rpm + hardened systemd unit
    ├── TPM on the runner                    -> Tpm2KeyMaterialProvider
    └── Central controller with KMS/Vault    -> a custom KeyMaterialProvider
```
