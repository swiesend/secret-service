...
- **Environment / cmdline scrape**: `/proc/<pid>/environ` reveals env-var peppers; `/proc/<pid>/cmdline` reveals plaintext password CLI flags. (See [LUKS / full-disk encryption](full-disk-encryption.md) in the implementation review for why the Tpm2Provisioner refuses to take `--password <plaintext>`.)
