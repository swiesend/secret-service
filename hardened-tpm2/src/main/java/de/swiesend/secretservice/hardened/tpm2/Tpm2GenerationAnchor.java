...
public final class Tpm2GenerationAnchor implements GenerationAnchor {
    ...
    public Tpm2GenerationAnchor(int nvIndex, char[] password, Supplier<Tpm> tpmSupplier) {
        ...
        try {
            ...
            // The construction failed, so close() will never run for this object -- zero the auth
            // here or it stays in the heap. The earlier failure path (TPM connection) does the same.
            Arrays.fill(auth, (byte) 0);
            closeQuietly();
