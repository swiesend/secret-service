 ...
 * {@code acknowledgeSameUidExposure(true)} because {@code PARTIAL} is not the
...
public final class Tpm2KeyMaterialProvider implements KeyMaterialProvider, AutoCloseable {
    ...
    public char[] getPepper() {
        ...
        char[] out = cachedPepper.clone();
        // Re-check after the copy: a close() racing this call could have zeroed the cache between
        // the guard and the clone, and an all-zero "pepper" seals items under a key nothing can
        // reproduce -- silent, permanent data loss where the guard was meant to raise.
        if (closed) {
            Arrays.fill(out, '\0');
