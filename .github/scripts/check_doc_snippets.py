...
import de.swiesend.secretservice.LogPolicy;
...
# A java fence, in any form MkDocs accepts: ```java, ~~~java, more than three markers, and any
# trailing attributes (```java title="Foo.java", ```java hl_lines="2 3"). Matching only the bare
# ```java form would make this checker SILENTLY SKIP a titled snippet -- the failure mode a
# guardrail must not have, since a skipped snippet looks exactly like a passing one.
JAVA_FENCE = re.compile(r"^(?P<marker>`{3,}|~{3,})java(?![\w-])(?P<attrs>.*)$")

def fences(md: pathlib.Path):
    lines = md.read_text().split("\n")
    i = 0
    while i < len(lines):
        opening = JAVA_FENCE.match(lines[i].strip())
        if opening:
            skip = None
            for back in (i - 1, i - 2):
                if back >= 0 and SKIP.search(lines[back]):
                    skip = SKIP.search(lines[back]).group(1).strip() or "(no reason given)"
            # Close on the same marker character, at least as long as the opener -- per CommonMark.
            # A ```` ```` ```` fence is how a snippet embeds a ``` block, so closing on any "```"
            # would truncate it mid-snippet and report a bogus syntax error.
            marker = opening.group("marker")
            close = re.compile(r"^" + re.escape(marker[0]) + "{" + str(len(marker)) + r",}\s*$")
            j = i + 1
            while j < len(lines) and not close.match(lines[j].strip()):
                j += 1
...
def main() -> int:
    ...
    for md in sorted(DOCS.rglob("*.md")):
        for n, (line, body, skip) in enumerate(fences(md)):
            ...
            # Name from the path RELATIVE TO docs/, not md.stem: docs/ holds four index.md files,
            # so a stem-based name collides across directories and one snippet silently overwrites
            # another's source file -- a missed check that reports as a pass.
            name = re.sub(r"\W", "_", f"S_{md.relative_to(DOCS).with_suffix('')}_{line}")
